 ...
 */
@WebServlet("/EventPagePost")
public class EventPagePostService extends HttpServlet {
	...
	// changeme			String OPENSHIFT_DATA_DIR= OPENSHIFT_DATA_DIR = "D:/data/";
		...
		String textarea_value = java.net.URLDecoder.decode(request.getParameter("textarea_value"));
		...
		String fileName = generator.nextSessionId()+"."+fileType;
        ...
        } catch (Exception e)
        {
	      ...
	      try
	      {
	    	  ...
	    	  //System.out.println(" img_src "+fileName+" datetime_value "+datetime_value+" textarea_value "+textarea_value+" select_value "+select_value); //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	    	  output = controller.executerEvent(fileName,textarea_value,select_value,pageInfo);

...
public class EventPostService extends HttpServlet {
	...
	// changeme			 String OPENSHIFT_DATA_DIR= OPENSHIFT_DATA_DIR = "D:/data/";
		...
		if(completeImageData.length()>1){
	      ...
	      try
	      {
	    	  ...
	    	  output = controller.eventExecuter(fileName,select_value,pageInfo,msg_value, linkUrl_value, linkImageUrl_value, linkDesc_value, linkTitle_value);
			 /* 
			  if(!fileName.isEmpty()){
